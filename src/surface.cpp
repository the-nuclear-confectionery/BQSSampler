...
void Surface::read_data()
{
    ...
    for (int i = 0; i < npoints; i++)
    {
        ...
        double u_dot_n = (gamma * aux_dsigma_t + aux_ux *aux_dsigma_x + aux_uy * aux_dsigma_y);
        if(i == 20424){ 
            std::cout << "u" << gamma << " " << aux_ux << " " << aux_uy << std::endl;
            std::cout <<"dat: " << aux_dsigma_t << " " << aux_dsigma_x << " " << aux_dsigma_y << std::endl;
        ...
        double ut_enforced = sqrt(1.0 + aux_ux * aux_ux + aux_uy * aux_uy + aux_tau * aux_tau * aux_ueta * aux_ueta);  // enforce normalization
        ...
        double dsigma_dsigma = dat * dat - dax * dax - day * day - dan * dan / (aux_tau * aux_tau);
